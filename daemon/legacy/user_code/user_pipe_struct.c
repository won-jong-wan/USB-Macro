...
typedef struct {
    uint32_t magic; // 잘못된 패킷이 왔는지 확인하는 역할
    uint8_t info;         // [7:4]version [3]type [2]isEnd [1:0]ZERO
    uint16_t cmd_len;
    char command[249];  // 실행할 명령어 문자열 저장
} __attribute__((packed)) datapacket;   // 구조체 패딩 제거 = 메모리 크기 최소화
...
void create_packet(datapacket *pkt, uint8_t version, uint8_t type, uint8_t isEnd, const char *cmd)
{
...
int main()
{
    ...
    create_packet(&packets[0], 1, 0, 0, "sudo apt update");
    create_packet(&packets[1], 1, 1, 0, "ping localhost");
    create_packet(&packets[2], 1, 0, 0, "sudo apt upgrade");
    create_packet(&packets[3], 1, 1, 1, "while true; do date; sleep 1; done");
    ...
    for(int i=0; i<packet_count; i++)
    {
        ...
        int result = process_packet(&packets[i]);

        if(result != 0 && GET_TYPE(packets[i].info) == 0)
        {
            printf("S노드 실패!!\n");
            break;
        }
