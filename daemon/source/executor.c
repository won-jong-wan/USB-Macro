...
static void internal_exec_s_node(const char *cmd) {
    ...
    // 1. 명령 전송 + 종료 마커 요청
    dprintf(active->fd_in, "%s; echo \"__END__:$?:$PWD\"\n", actual_cmd);
...
static void internal_exec_c_node(const char *cmd) {
    ...
    if (pid == 0) {
        // 자식 프로세스
        if (!is_sudo) {
            // [중요] C-Node도 환경변수 및 권한 맞춰줌
            setenv("HOME", target_home, 1);
            setresgid(target_gid, target_gid, target_gid);
            setresuid(target_uid, target_uid, target_uid);
            
            // 현재 S-Node의 User 쉘이 있는 경로에서 실행하고 싶다면?
            chdir(user_sh.current_dir); // 옵션
        } else {
            setenv("HOME", "/root", 1);
            chdir(root_sh.current_dir); // 옵션
        }
        
        execl("/bin/sh", "sh", "-c", actual_cmd, NULL);
        exit(1);
    }
}
